...
// transitioning its own `stroke-dashoffset` to zero. pathLength is normalized to
// 1 on every stroke so the dash math is independent of the traced geometry.
//
...
// is a single, continuous pen gesture with its own start delay, duration (ms),
// and easing, so the reveal follows the pen rather than flickering on at random.
// Strokes that belong to one letter run back to back (delay = the previous
// stroke's delay + duration, no gap); the small jumps in delay between names are
// the held beats.
//
...
const STROKES = [
  ...
  {
    d: "M52.8 14.8C53.1 14.7 54.4 13.7 53.5 13.6C53.1 13.5 52.6 13.8 52.3 14.1C51.4 14.7 50.6 15.5 49.8 16.1C46.2 19 43.1 22.3 40 25.6C37 28.8 34.2 31.9 31.5 35.2C29.8 37.4 28.2 39.6 26.5 41.8C23.4 45.7 20.1 49.4 17.1 53.5C14.3 57.4 11.5 61.3 8.6 65.1C7.7 66.4 6.6 67.7 5.7 69C5.4 69.4 5.4 69.9 5 70.3",
    delayMs: 0,
    durationMs: 190,
    ease: G_EASE,
  },
  ...
  {
    d: "M20.5 79.1C24.9 76.6 29.6 74.7 34.3 72.8C38.1 71.2 41.9 69.7 45.8 68.4C49.7 67 54 65.3 58.2 64.9C60.4 64.6 62.5 64.7 64.5 65.9C66.2 66.8 67.5 68.5 69.5 69C73.1 69.9 76.8 67.7 79.9 66.1C81.4 65.3 83 64.7 84.4 63.7",
    delayMs: 380,
    durationMs: 170,
    ease: G_EASE,
  },
  // "iacomo", left to right (after a beat).
  {
  ...
  // Guidotto — capital G, drawn alone on its own beat: body then underline
  // sweep, back to back.
  {
  ...
  {
    d: "M154.8 74.9C163.4 74.9 171.9 72.3 180.3 71.1C185.7 70.3 191.2 69.5 196.7 69C200.6 68.6 204.5 68.3 208.3 67.7C212.6 67 216.8 66.2 221.1 65.6C223.5 65.3 225.9 64.8 228.4 64.8",
    delayMs: 1370,
    durationMs: 190,
    ease: G_EASE,
  },
  // "uidotto", flowing in letter by letter after a beat.
  {
    d: "M198.5 47.9C198.2 48.7 197.3 49.6 197.3 50.5C197.4 51.4 199 51.2 199.6 51.1C201.8 50.9 203.3 48.8 204.7 47.4C205.7 48.4 204.7 50 205.3 51.4C205.7 52.2 206.8 52.4 207.6 52.1C209 51.5 209.6 49.9 210.5 48.8C210.8 49.1 211.1 49.3 211.1 49.7C211.2 50.3 210.9 51.4 211.5 51.8C212 52.2 212.7 51.5 213 51.2C213.7 50.5 214.8 47.9 215.9 48C216.5 48 216.7 48.6 216.5 49.1C216.1 49.8 215.2 50.3 214.8 51C214.5 51.3 214.5 51.8 215 51.9C216.2 52.3 218.1 50.4 218.8 49.6C220.3 47.9 221.7 46.3 222.9 44.3C223.7 43.1 224.4 41.4 225.7 40.6",
    delayMs: 1680,
    durationMs: 150,
    ease: PEN_EASE,
  },
  {
    d: "M230.9 29.9C230.7 31.8 229 33.8 228.2 35.6C226.4 39.4 225 43.4 223.4 47.4C222.8 48.8 222.2 50.2 221.6 51.6C221.4 52.1 221 52.8 221.2 53.3C221.4 53.7 222 53.5 222.3 53.4C223.2 52.9 224.1 52.4 224.9 51.8C227.1 50.1 229 48.3 230.9 46.3C231.8 45.3 232.9 43.6 234.2 43.1C235 42.8 235 43.6 234.9 44.1C234.7 45.1 234 46.1 233.4 46.9C232.5 48 231.8 48.9 231.2 50.2",
    delayMs: 1830,
    durationMs: 140,
    ease: PEN_EASE,
  },
  {
    d: "M230.2 51.3C231.2 50.4 232.4 50.1 233.5 49.3C234.5 48.5 235.2 47.4 236 46.4C236.9 45.4 237.8 44.5 238.6 43.4C240.7 40.4 242.1 37 244.4 34.1C244.6 33.8 244.4 33.4 244.6 33.1C244.9 32.8 246 33 246.4 32.9C247.8 32.8 249.2 32.8 250.5 32.7C251.1 32.6 251.8 32.8 252.3 32.5C252.9 32.2 253.3 30.6 253.6 30C254.3 28.4 254.9 26.9 255.5 25.3C255.8 24.4 256 23.4 256.5 22.6",
    delayMs: 1970,
    durationMs: 130,
    ease: PEN_EASE,
  },
  {
    d: "M234 33.4L241.5 33.2L244.5 33.2",
    delayMs: 2100,
    durationMs: 80,
    ease: PEN_EASE,
  },
...
// Dash geometry for arming each stroke. With pathLength normalized to 1, a dash
// of length 1 covers the whole stroke; the longer trailing gap lets the armed,
// hidden state park the stroke deep inside that gap (ARMED_OFFSET) so neither
// round end cap pokes back onto the path as a stray dot. The reveal touches the
// pen down at DRAW_OFFSET (the dash's leading edge exactly at the stroke start)
// and runs to 0.
const DASH = "1 2";
const ARMED_OFFSET = "1.5";
const DRAW_OFFSET = "1";
...
export function Signature() {
  ...
  useEffect(() => {
    ...
    // Arm every stroke: park it, with no transition, deep inside the dash gap so
    // nothing shows and nothing moves until the signature is scrolled into view.
    for (const path of paths) {
      ...
      path.style.strokeDashoffset = ARMED_OFFSET;
    }
    ...
    // Each stroke's draw is scheduled by its own timer so it can sit safely parked
    // until its turn (rather than holding at the pen-down position, where a cap
    // dot would show). Tracked here so the cleanup can cancel any still pending.
    ...
    const observer = new IntersectionObserver(
      (entries, obs) => {
        for (const entry of entries) {
          ...
          // Draw the strokes on, once, in order: each touches down at its delay
          // and runs for its own duration/easing, so the held beats and the final
          // slow-down live entirely in the STROKES data.
          paths.forEach((path, i) => {
            ...
            timers.push(
              setTimeout(() => {
                // Touch the pen down (jump to the stroke start with no
                // transition), force the style to commit, then draw to the end.
                path.style.transition = "none";
                path.style.strokeDashoffset = DRAW_OFFSET;
                path.getBoundingClientRect();
                path.style.transition = `stroke-dashoffset ${durationMs}ms ${ease}`;
